...
class CourtFail:
    """Анимация неудачного броска - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
    ...
    def draw_court(self):
        ...
        with canvas.canvas:
            ...
            # === МЯЧ - СПРАВА ОТ ИГРОКА ===
            self.ball_x = man_x + man_size_w * 0.5
            self.ball_y = man_y + man_size_h * 0.2
            
            self.target_x = w * 0.78
            self.target_y = offset_y + ground_height * 0.55
            self.step = w * 0.025
            self.fall_speed = h * 0.025
            self.bounce_height = offset_y + ground_height * 0.35
            self.gravity = h * 0.0025
    ...
    def move_ball(self, dt):
        ...
        try:
            ...
            if dist < 10 and self.ball_y > self.target_y - 10:
                self.bouncing = True
                self.fall_speed = -self.canvas.height * 0.035
